...
+(NSMutableArray*)myAddObject:(NSString*)myid:(NSMutableArray*)muArray{
    ...
    for(int i=0; i<[muArray count]; i++) {
        NSDictionary *object = [muArray objectAtIndex: i];
    ...
    if (!getroffen){
        NSDictionary *qDictionary = [[NSDictionary alloc]
                                     initWithObjectsAndKeys:myid,@"id",                                                                         nil];
        [muArray addObject:qDictionary];
    }
...
+ (NSString*)stringAfter:(NSString *)str{
    NSString *result = @"";
    NSRange equalRange = [str rangeOfString:@"gotoartikel=" options:NSBackwardsSearch];
